...
def test_conflict_graph_edges_cover_each_category():
    ...
    # Post-v1 (Unit 68): unit/session overlap is not generated by
    # build_conflict_graph; student overlap from allocation data subsumes it.
    # Different lecturers means no lecturer-overlap edge either.
    assert ConstraintType.STUDENT_OVERLAP in kinds
    assert ConstraintType.LECTURER_OVERLAP not in kinds
    assert ConstraintType.UNIT_SESSION_OVERLAP not in kinds
...
def test_snapshot_builder_compiles_real_format_data():
    ...
    # Same lecturer yields a lecturer-overlap pair.
    assert ("s-free", "s-locked") in [
        tuple(sorted(p)) for p in snapshot.lecturer_conflict_pairs
    ]
    # Post-v1 (Unit 68): unit_session_conflict_pairs is empty — not generated.
    assert snapshot.unit_session_conflict_pairs == []
...
def test_solver_same_unit_no_conflict_post_v1():
    """Post-v1 (Unit 68): two sessions from the same unit with different
    session-level lecturers and no shared allocated students are NOT in
    conflict — both can be scheduled simultaneously in separate rooms."""
    sessions = [
        session_snapshot("s-a", unit_id="unit-a", lecturer_id="lec-1"),
    ...
    result = solve_timetable(snapshot, time_limit_seconds=SOLVE_LIMIT)
    # No unit/session conflict pair generated; disjoint allocated students,
    # different lecturers → both sessions scheduled simultaneously.
