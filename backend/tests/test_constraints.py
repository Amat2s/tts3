...
def test_conflict_graph_conflicts_between(session_a1, session_a2):
    ...
    # Post-v1 (Unit 68): unit/session overlap not generated by build_conflict_graph.
    # Same lecturer → LECTURER_OVERLAP. Shared students → STUDENT_OVERLAP.
    assert ConstraintType.LECTURER_OVERLAP in types
    assert ConstraintType.STUDENT_OVERLAP in types
    assert ConstraintType.UNIT_SESSION_OVERLAP not in types
