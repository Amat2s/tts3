...
33. Lecturer scheduling preferences (Unit 98) are persisted as room-specific soft-constraint cells in `lecturer_preferences`, one row per `lecturer_id + day + slot + room_id` carrying exactly one `level` (`preferred` or `avoid`). No row means neutral (no preference); neutral is never stored. A unique `(lecturer_id, day, slot, room_id)` keeps a cell to a single level, and rows cascade when their lecturer or room is deleted. Preferences are distinct from lecturer availability and timetable blocks (both hard constraints), are not sessions, and never appear in session/scheduling models. The backend persists submitted cells as-is: it validates only that the referenced lecturer and room exist and that `level` is one of the two allowed values, and performs no cross-validation against availability, blocks, or existing sessions. Protected CRUD lives on `GET /lecturers/{lecturer_id}/preferences`, `PUT /lecturer-preferences` (upsert one cell by its four-tuple), and `DELETE /lecturer-preferences` (remove one cell back to neutral). Solver soft-constraint integration is a later unit — this contract adds no solver behaviour.
