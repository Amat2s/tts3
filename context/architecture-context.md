...
- `solver/` — OR-Tools integration layer inside backend responsible for converting saved sessions + mirrored solver constraints into a CP-SAT model. It consumes blocked cells from the snapshot so the model never occupies a blocked `day + slot + room` cell, and consumes lecturer preference cells as a strictly-secondary objective tie-breaker (never a feasibility constraint).
...
- **Supabase PostgreSQL**: Stores all core domain data:
  ...
  - room-specific lecturer scheduling preference cells (`lecturer_preferences`)
...
33. Lecturer scheduling preferences (Unit 98) are persisted as room-specific soft-constraint cells in `lecturer_preferences`, one row per `lecturer_id + day + slot + room_id` carrying exactly one `level` (`preferred` or `avoid`). No row means neutral (no preference); neutral is never stored. A unique `(lecturer_id, day, slot, room_id)` keeps a cell to a single level, and rows cascade when their lecturer or room is deleted. Preferences are distinct from lecturer availability and timetable blocks (both hard constraints), are not sessions, and never appear in session/scheduling models. The backend persists submitted cells as-is: it validates only that the referenced lecturer and room exist and that `level` is one of the two allowed values, and performs no cross-validation against availability, blocks, or existing sessions. Protected CRUD lives on `GET /lecturers/{lecturer_id}/preferences`, `PUT /lecturer-preferences` (upsert one cell by its four-tuple), and `DELETE /lecturer-preferences` (remove one cell back to neutral). The frontend `/preferences` editor is immediate-persist (no dirty-draft/save step, no browser-storage draft): the frontend owns the on-screen grid state and the backend only stores each click (Unit 100/103).
34. Lecturer preferences are the **one** soft constraint in the v1 solver (Unit 101), which otherwise stays a pure hard-constraint model. The solver input snapshot (`SolverInputSnapshot`) carries a `preferences: list[PreferenceSnapshot]` field (`PreferenceSnapshot(lecturer_id, day, slot, room_id, level)`); the DB loader reads every `LecturerPreference` row with no cross-validation and no integrity rejection (unlike locked assignments/blocks, preferences carry no feasibility meaning), sorted deterministically. Preferences never gate candidate generation or any feasibility check (capacity, lunch, off-timetable, availability, locked occupancy, blocks). They enter only the objective, which becomes lexicographic: `primary_multiplier · Σ(scheduled) + Σ(preference_score · var)`, where each candidate's `preference_score` is `+1` per `preferred` occupied cell and `−1` per `avoid` occupied cell (one fixed uniform weight), and `primary_multiplier` is chosen strictly larger than the maximum possible secondary swing. Scheduling one more session therefore always dominates any preference score: preferences only break ties among equally-maximal outcomes and can never reject a feasible placement, reduce the scheduled count, or alter result application. Determinism is preserved (`num_search_workers=1`, `random_seed=0`, deterministic build order).
