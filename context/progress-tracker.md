...
- Post-v1 work is progressing through the build plan: Units 58–68 are now complete. Unit 68 (just completed) updated the backend solver stack to use session-level lecturers and per-session allocation data — the constraint mirror no longer generates unit/session overlap edges (student overlap from allocations subsumes it), the snapshot builder loads `session_student_allocations` per session instead of `unit.students`, and the result-application service validates capacity against the allocation count. Remaining post-v1 units: the remaining frontend route/validation units (69–71). Note one carried-forward gap still open: the backend `SchedulableSessionResponse` does not yet emit `unit_year_level` (typed optional on the frontend, to be addressed when a later unit needs it).
