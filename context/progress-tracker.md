...
- Post-v1 work is progressing through the build plan: Unit 58 added three-year operation (derived unit year levels + year-matched enrolment sync), Unit 59 added unit teaching teams + session-level lecturers, Unit 60 reduced session types to lecture/tutorial and introduced hidden session-student allocations (lecture = all enrolled, tutorial = even split), and Unit 61 hardened lecturer availability persistence into a transactional replace-all save. Unit 62 aligned the frontend API clients/DTO types to that new backend model without redesigning routes, and Unit 63 (just completed) redesigned the `/units` page on top of those types — wider two-column modal, derived read-only year level, year-aware student search/filter selection, multi-lecturer teaching team, per-session lecturer selection, and a 1–4 duration stepper labelled in hours. Unit 64 added student enrolment management on `/students` (reusing the shared unit-student relationship), and Unit 65 (just completed) added read-only lecturer teaching visibility on `/lecturers` — a "Units taught" table column and a read-only edit-modal summary, both derived from the units list, with teaching edits remaining owned by `/units`. Unit 66 (just completed) added frontend-only client-side search and filters to all four management pages (`/rooms`, `/students`, `/lecturers`, `/units`) via reusable `SearchInput`/`FilterSelect`/`FilterBar` primitives and pure per-domain filter helpers, with distinct true-empty vs no-match empty states and no backend/query-param/persistence change. Unit 67 (just completed) re-pointed frontend timetable validation at the new model: manual scheduling now judges room capacity against allocation-derived `student_count`, lecturer overlap/availability against session-level `lecturer_id`, and student overlap against `allocated_student_ids` set intersection (replacing the removed independent `unit_session_overlap` warning) — all inside `frontend/src/lib/validation` + `frontend/src/features/timetable`, blocking-vs-warning severities and explicit-save preserved. Remaining post-v1 units: the remaining frontend route/validation units (69–71) and backend Unit 68 (solver allocation + session-lecturer integration). Note one carried-forward gap surfaced in Unit 62 and still open after Unit 67: the backend `SchedulableSessionResponse` does not yet emit `unit_year_level` (typed optional on the frontend, and threaded as optional through `TimetableAssignment` for later filters), to be addressed when a later unit needs it.
