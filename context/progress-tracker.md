...
- v1 acceptance is complete and documented (Unit 53). Deployment wiring is being prepared in-repo across Units 54 (Vercel), 55 (Railway), and 56 (Trigger.dev production), each with the repo-side config + docs done and the remaining steps being manual dashboard/CLI operations. Candidate follow-ups (out of current scope): a fresh interactive human click-through against the running stack, executing the manual deploy steps, WebSocket live solver progress (architecture-context lists Realtime), and frontend bundle code-splitting.
